      ...
      >
        ...
        >
          {entry?.modular_blocks?.map((block, index) => (
            <div key={index} className={(block.text_block?.sticky || block.cards?.sticky || block.review?.sticky || block.text_and_image?.sticky || block.category_banner?.sticky) ? 'sticky top-0' : 'relative'} style={{ zIndex: index + 1 }} {...entry?.$?.["modular_blocks__" + index]}>
