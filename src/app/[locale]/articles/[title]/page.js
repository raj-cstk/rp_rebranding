    ...
    >
      ...
      <div className="pt-5 flex flex-col flex-1">
        ...
        <Link href={article.url || '#'}>
          <h2
            style={{ fontFamily: '"Cormorant Garamond", var(--font-cormorant-garamond), Georgia, serif', fontWeight: 300, fontStyle: 'italic', fontSize: 'clamp(1.25rem, 1.8vw, 1.6rem)', lineHeight: 1.25, color: hovered ? '#1a1410' : '#2a2420', transition: 'color 0.3s', marginBottom: '0.75rem' }}
            {...article.$?.title}
          >
            {article.title}
          </h2>
        ...
        {article.teaser && (
          <p className="line-clamp-3 flex-1" style={{ fontFamily: 'var(--font-raleway), sans-serif', fontWeight: 300, fontSize: '0.88rem', lineHeight: 1.85, color: '#6b6560' }} {...article.$?.teaser}>
            {article.teaser}
          </p>
        )}
...
export default function ArticlesList() {
  ...
  const getContent = async () => {
    ...
    if (first?.taxonomy_category?.length > 0) {
      const filtered = await ContentstackClient.getElementByTypeByTaxonomy(
        "article",
        params.locale,
      ...
      <div className="w-full px-8 md:px-16 py-20">
        <div className="max-w-7xl mx-auto">
          {entry?.taxonomy_category?.length === 0 ? (
            <div className="flex items-center justify-center py-32" {...entry?.$?.taxonomy_category}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-16" {...entry?.$?.taxonomy_category}>
